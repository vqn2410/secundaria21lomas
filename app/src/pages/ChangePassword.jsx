...
export default function ChangePassword() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // 3. Redirigir según el rol del usuario (esto lo podemos detectar o dejar que el login se encargue)
      // Por simplicidad redirigimos al login para que entre "limpio"
      auth.signOut();
      navigate('/login');
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="login-container" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '100vh', background: 'var(--bg)' }}>
      <div className="login-card" style={{ maxWidth: '400px', width: '90%', background: 'white', padding: '2.5rem', borderRadius: '16px', boxShadow: '0 10px 25px rgba(0,0,0,0.05)' }}>
        <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
          ...
          <h1>Cambio de Clave Obligatorio</h1>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
          ...
          <button type="submit" disabled={loading} className="btn btn-admin" style={{ marginTop: '1rem', width: '100%', padding: '1rem', borderRadius: '8px' }}>
