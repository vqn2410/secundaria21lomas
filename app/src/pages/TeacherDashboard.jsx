...
export default function TeacherDashboard() {
  ...
  const handleCreateStudent = async (e) => {
    ...
    try {
      ...
      setShowModal(false);
      setNewStudent({ nombre: '', apellido: '', email: '', dni: '' });
    ...
    <div className="dashboard">
      ...
      <main className="main-content">
        ...
        <section className="content-area">
          ...
          {/* Modal Asignación de Estudiante a Práctica */}
          {assigningTo && (
            <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
              <div className="card" style={{ maxWidth: '500px', width: '90%' }}>
                <h2 style={{ marginBottom: '1rem' }}>Asignar a: {assigningTo.apellido}, {assigningTo.nombre}</h2>
                <div className="form-group" style={{ marginBottom: '1.5rem' }}>
                  <label>Seleccionar Práctica / Materia</label>
                  <select className="input-field" value={selectedPracticeId} onChange={e => setSelectedPracticeId(e.target.value)}>
                    <option value="">Seleccione una práctica...</option>
                    {practices.map(p => (
                      <option key={p.id} value={p.id}>
                        {p.subject} - {p.day} ({p.time}) - {p.shift}
                      </option>
                    ))}
                  </select>
                  <small style={{ color: 'var(--text-light)', marginTop: '0.5rem', display: 'block' }}>
                    Solo aparecen las prácticas creadas por el Administrador.
                  </small>
                </div>
                <div style={{ display: 'flex', gap: '1rem' }}>
                    <button className="btn btn-teacher" style={{ flex: 1 }} onClick={handleAssignToPractice} disabled={loading || !selectedPracticeId}>
                        {loading ? 'Asignando...' : 'Confirmar Asignación'}
                    </button>
                    <button className="btn" style={{ flex: 1, border: '1px solid var(--border)' }} onClick={() => { setAssigningTo(null); setSelectedPracticeId(''); }}>
                        Cancelar
                    </button>
                </div>
              </div>
